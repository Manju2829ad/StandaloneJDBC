...
import com.jdbc.utility.JdbcUtil;
...
public class StudentRepoImpl implements IStudentRepo {
	...
	private static final String SQL_INSERT_QUERY = "insert into student_info(`sname`,`sage`,`saddress`,`status`) values(?,?,?,?) ";
	private static final String SQL_SELECT_QUERY = "select sid,sname,sage,saddress from student_info where sid =? ";
	private static final String SQL_DELETE_QUERY = "delete from student_info where sid = ? ";
	private static final String SQL_UPDATE_QUERY = "update student_info set sname=?,sage=?,saddress=?,status=? where sid =?";
